...
version is the index schema version. The server depends on `^3.2.0`. The major is the
schema it reads, and `3.2.0` is a floor: `MIN_DATA_VERSION` in `src/db.ts`, the first
...
`npc_destination.origin` did in `3.1.0` and `creature.race_id` in `3.2.0`, so an older
index of the same major would pass npm and then fail the schema probe at startup. The
floor keeps npm from installing it.
...
The caret is a deliberate exception to this repository's exact pins. An exact `3.2.0`
...
server release. `pnpm add` does not write `^3.2.0`, so edit the range by hand.
