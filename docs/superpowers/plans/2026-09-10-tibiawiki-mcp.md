...
- **Exact dependency pins** (pnpm `savePrefix: ''`), all older than the 7-day `minimumReleaseAge` gate: `@modelcontextprotocol/server@2.0.0`, `@modelcontextprotocol/client@2.0.0` (dev), `zod@4.5.4`, `typescript@7.0.2`, `@types/node@24.13.3`.
- **No `@modelcontextprotocol/inspector` dependency.** Adding it was blocked at install by `trustPolicy: no-downgrade`: its transitive `chokidar@4.0.3` has no provenance attestation while `4.0.0`/`4.0.1` do (`ERR_PNPM_TRUST_DOWNGRADE`). Rather than weaken a security policy for a dev tool, protocol conformance is tested directly with `StdioClientTransport` from `@modelcontextprotocol/client` — already a dependency — spawning the real `dist/index.js`. This is strictly better: it exercises the actual shipped binary over the actual stdio transport, inside `pnpm test` rather than only in CI, and drops ~100 transitive dev packages (18 installed instead). **Do not bump to `zod@4.6.1`, `@types/node@24.13.4`, or `@modelcontextprotocol/inspector@2.6.0`** — all published 2026-09-09; with exact pins and `minimumReleaseAge: 10080` there is no fallback and `pnpm install` hard-fails with `ERR_PNPM_NO_MATURE_MATCHING_VERSION`. When choosing any pin, take one with **margin** rather than one sitting exactly on the 7-day line: the gate is computed in hours, so `inspector@2.5.0` (published 2026-09-02, exactly 7 days) is a coin flip — hence `2.4.0`. Verified ages on 2026-09-10: zod 4.5.4 = 11d, @types/node 24.13.3 = 63d, typescript 7.0.2 = 63d, both `@modelcontextprotocol/*@2.0.0` = 44d, inspector 2.4.0 = 14d.
...
| Item stat values | **Not always integers.** Three *active* items carry a bonus suffix — `Ink Sword` and both Moonsilver Axes have `defense` of `"12 +1"` / `"33 +3"`. `Number()` on those is `NaN`, which the SDK-enforced `outputSchema` rejects, failing the whole call. Coercion is best-effort in `domain.ts`: a clean integer becomes a number, anything else stays a string. SQLite's `cast('33 +3' as integer)` is 33, so filtering stays consistent |
...
- `pnpm test` passes.
- **Do not run `pnpm build` in this task.** With no `src/` yet it exits 2 with `TS18003: No inputs were found` — correct behaviour, not a failure to fix. The build and the `dist/index.js` gate both belong to **Task 4**, where `src/index.ts` first exists. Task 1 asserts only that `tsconfig.build.json` declares `rootDir: "src"` (the setting that later makes the emitted path flat).
