...
export function readPng(bytes) {
    ...
    while (pos < bytes.length) {
        ...
        if (type === 'IHDR') {
            ...
            if (depth !== 8 || body[12] !== 0)
                throw new Error(`Unsupported PNG: depth ${depth}, interlace ${body[12]}.`);
        }
        else if (type === 'PLTE')
            palette = body;
        else if (type === 'tRNS')
    ...
    for (let y = 0; y < height; y += 1) {
        ...
        for (let x = 0; x < width; x += 1) {
            ...
            const d = (y * width + x) * 4;
            if (colour === 6)
                out.set(line.subarray(s, s + 4), d);
            else if (colour === 2) {
                out.set(line.subarray(s, s + 3), d);
                out[d + 3] = 255;
            }
            else if (colour === 4) {
                out.fill(line[s], d, d + 3);
                out[d + 3] = line[s + 1];
            }
            else if (colour === 0) {
                out.fill(line[s], d, d + 3);
                ...
                const i = line[s];
                out[d] = palette[i * 3];
                out[d + 1] = palette[i * 3 + 1];
                out[d + 2] = palette[i * 3 + 2];
 ...
 */
export function extractWebpFrames(webp) {
    const view = new DataView(webp.buffer, webp.byteOffset, webp.byteLength);
    const tag = (at) => String.fromCharCode(...webp.subarray(at, at + 4));
    if (tag(0) !== 'RIFF' || tag(8) !== 'WEBP')
        throw new Error('Not a WebP container.');
    const out = [];
    const u24 = (at) => webp[at] | (webp[at + 1] << 8) | (webp[at + 2] << 16);
 ...
 */
export function classify(background, deltas, opts = {}) {
    ...
    let best = { width: cols, height: rows, cells: new Array(cols * rows).fill(0) };
    let bestCount = -1;
    for (const delta of deltas) {
        ...
        for (let y = 0; y < delta.height; y += 1) {
            for (let x = 0; x < delta.width; x += 1) {
                ...
                if (delta.pixels[s + 3] <= 16)
                    continue;
                const [gx, gy] = [delta.x + x, delta.y + y];
                if (gx < 0 || gy < 0 || gx >= background.width || gy >= background.height)
                    continue;
                ...
                if (diff > tolerance)
                    counts[Math.floor(gy / tile) * cols + Math.floor(gx / tile)] += 1;
            }
        }
        const max = Math.max(...counts);
        if (max === 0)
            continue;
        const cells = counts.map((c) => (c > max * relative ? 1 : 0));
        const hit = cells.reduce((n, c) => n + c, 0);
        if (hit > bestCount) {
            bestCount = hit;
            best = { width: cols, height: rows, cells };
        }
    }
    return best;
}
//# sourceMappingURL=spell-decode.js.map